...
// Cria e tria uma transacao na Didit (server-to-server REST, sem SDK).
export async function createDiditTransaction(params: {
  transactionRef: string; // nosso id unico (<=128 chars)
  direction: "inbound" | "outbound";
  amount: number;
  currency?: string; // ISO 4217, ex BRL
  userId: string; // vendor_data: nosso id interno
  userFullName: string;
  ...
  const subject: Record<string, unknown> = {
    ...
    full_name: params.userFullName || "N/A",
  };
  ...
  if (params.counterpartyName || params.counterpartyAccount) {
    payload.counterparty = {
      full_name: params.counterpartyName || "N/A",
      payment_method: {
        method_type: params.counterpartyMethod || "bank_account",
        account_id: params.counterpartyAccount || "N/A",
      },
    };
  }

  const res = await fetch(`${DIDIT_BASE_URL}/v3/transactions/`, {
    method: "POST",
    headers: {
      "x-api-key": apiKey,
      "Content-Type": "application/json",
  ...
  if (!res.ok) {
    const detail = await res.text();
    throw new Error(`Didit transaction_create_failed (${res.status}): ${detail}`);
  }

  return (await res.json()) as DiditTransactionResult;
}
